...
- `boundaries.py` — `grid_boundaries_from_mask` (inverse: recover boundary faces from a mask). Plain single-tile grids trace with `contourpy`; single-tile **fold** grids (`_fold_boundaries_from_mask`, detected via `_has_fold`) also trace with `contourpy` but drop fold-seam segments interior to the region and stitch the two sides into one loop by seam coincidence; multi-tile grids trace per-face and stitch across tile seams using the grid's `face_connections` topology, returning a per-corner face index (`f_c_list`, `None` for single-tile incl. fold).
